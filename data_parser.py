...
def save_data(file, data_selected):
    ...
    for data_name in data_selected:
        ...
        for x in data_type.x:
            if x not in dictionary["time(ms)"]:
                dictionary["time(ms)"].append(x)
    dictionary["time(ms)"].sort()
    for data_name in data_selected:
        data_type = title2data([data_name])[0]
        key = data_type.title + " (" + data_type.units + ")"
        dictionary[key] = []
        for x in dictionary["time(ms)"]:
            if x in data_type.x:
                index = np.where(data_type.x == x)[0][0]
                dictionary[key].append(data_type.y[index])
            else:
